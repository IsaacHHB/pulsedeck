...
for (let y = 0; y < size; y++) for (let x = 0; x < size; x++) {
  const sx = x * 256 / size, sy = y * 256 / size;
  const dx = Math.max(38 - sx, 0, sx - 217), dy = Math.max(38 - sy, 0, sy - 217);
  const inside = dx * dx + dy * dy <= 38 * 38;
  let rgba = inside ? [208, 247, 139, 255] : [0, 0, 0, 0];
  const heights = [70, 114, 152, 94, 62];
  for (let bar = 0; bar < 5; bar++) if (sx >= 56 + bar * 30 && sx < 73 + bar * 30 && sy >= (256 - heights[bar]) / 2 && sy < (256 + heights[bar]) / 2) rgba = [36, 51, 27, 255];
...
fs.writeFileSync(path.join(__dirname, 'icon.png'), makePng(1024));
