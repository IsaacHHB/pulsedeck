...
async function main() {
  ...
  const executablePath = process.env.PULSEDECK_EXE || path.resolve(__dirname, '..', 'dist', 'win-unpacked', 'PulseDeck.exe');
  const app = await electron.launch({ executablePath, args: ['--no-sandbox'], env, timeout: 30000 });
  try {
    ...
    console.log(JSON.stringify({ ...info, devices, note: 'Read-only device enumeration. No microphone stream was requested. Package launched with test-only --no-sandbox due to execution environment restrictions.' }, null, 2));
    await fs.writeFile(path.join(results, 'package-report.json'), JSON.stringify({ ...info, devices, sandboxDisabledForAutomation: true, captureStarted: false }, null, 2));
